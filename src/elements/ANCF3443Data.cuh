...
struct GPU_ANCF3443_Data : public ElementBase {
    ...
    //   d_fixed_nodes[] and x_cur_jac/y_cur_jac/z_cur_jac targets.
    ...
    __device__ const Map<VectorXR> gauss_xi_m() const { return Map<VectorXR>(d_gauss_xi_m, Quadrature::N_QP_7); }

    __device__ const Map<VectorXR> gauss_eta_m() const { return Map<VectorXR>(d_gauss_eta_m, Quadrature::N_QP_7); }

    __device__ const Map<VectorXR> gauss_zeta_m() const { return Map<VectorXR>(d_gauss_zeta_m, Quadrature::N_QP_3); }

    __device__ const Map<VectorXR> gauss_xi() const { return Map<VectorXR>(d_gauss_xi, Quadrature::N_QP_4); }

    __device__ const Map<VectorXR> gauss_eta() const { return Map<VectorXR>(d_gauss_eta, Quadrature::N_QP_4); }

    __device__ const Map<VectorXR> gauss_zeta() const { return Map<VectorXR>(d_gauss_zeta, Quadrature::N_QP_3); }

    __device__ const Map<VectorXR> weight_xi_m() const { return Map<VectorXR>(d_weight_xi_m, Quadrature::N_QP_7); }

    __device__ const Map<VectorXR> weight_eta_m() const { return Map<VectorXR>(d_weight_eta_m, Quadrature::N_QP_7); }

    __device__ const Map<VectorXR> weight_zeta_m() const { return Map<VectorXR>(d_weight_zeta_m, Quadrature::N_QP_3); }

    __device__ const Map<VectorXR> weight_xi() const { return Map<VectorXR>(d_weight_xi, Quadrature::N_QP_4); }

    __device__ const Map<VectorXR> weight_eta() const { return Map<VectorXR>(d_weight_eta, Quadrature::N_QP_4); }

    __device__ const Map<VectorXR> weight_zeta() const { return Map<VectorXR>(d_weight_zeta, Quadrature::N_QP_3); }

    // ==============================================================

    __device__ Map<VectorXR> x_cur_jac() { return Map<VectorXR>(d_x_cur_jac, n_coef); }

    __device__ Map<VectorXR> const x_cur_jac() const { return Map<VectorXR>(d_x_cur_jac, n_coef); }

    __device__ Map<VectorXR> y_cur_jac() { return Map<VectorXR>(d_y_cur_jac, n_coef); }

    __device__ Map<VectorXR> const y_cur_jac() const { return Map<VectorXR>(d_y_cur_jac, n_coef); }

    __device__ Map<VectorXR> z_cur_jac() { return Map<VectorXR>(d_z_cur_jac, n_coef); }

    __device__ Map<VectorXR> const z_cur_jac() const { return Map<VectorXR>(d_z_cur_jac, n_coef); }

    __device__ Map<VectorXR> x_cur() { return Map<VectorXR>(d_x_cur, n_coef); }

    __device__ Map<VectorXR> const x_cur() const { return Map<VectorXR>(d_x_cur, n_coef); }

    __device__ Map<VectorXR> y_cur() { return Map<VectorXR>(d_y_cur, n_coef); }

    __device__ Map<VectorXR> const y_cur() const { return Map<VectorXR>(d_y_cur, n_coef); }

    __device__ Map<VectorXR> z_cur() { return Map<VectorXR>(d_z_cur, n_coef); }

    __device__ Map<VectorXR> const z_cur() const { return Map<VectorXR>(d_z_cur, n_coef); }
    ...
    __device__ void x_cur_elem(int elem, Real* buffer) const {
        gather_element_dofs(d_x_cur, this->element_connectivity(), elem, buffer);
    }

    __device__ void x_cur_jac_elem(int elem, Real* buffer) const {
        gather_element_dofs(d_x_cur_jac, this->element_connectivity(), elem, buffer);
    }

    // Accessor for y_cur for a given element
    __device__ void y_cur_elem(int elem, Real* buffer) const {
        gather_element_dofs(d_y_cur, this->element_connectivity(), elem, buffer);
    }

    __device__ void y_cur_jac_elem(int elem, Real* buffer) const {
        gather_element_dofs(d_y_cur_jac, this->element_connectivity(), elem, buffer);
    }

    // Accessor for z_cur for a given element
    __device__ void z_cur_elem(int elem, Real* buffer) const {
        gather_element_dofs(d_z_cur, this->element_connectivity(), elem, buffer);
    }

    __device__ void z_cur_jac_elem(int elem, Real* buffer) const {
        gather_element_dofs(d_z_cur_jac, this->element_connectivity(), elem, buffer);
    }

    __device__ Map<MatrixXi> element_connectivity() const { return Map<MatrixXi>(d_element_connectivity, n_beam, 4); }
    ...
    __device__ const Real* constraint_rhs() const { return d_constraint_rhs; }

    __device__ int constraint_mode_device() const { return constraint_mode; }

    __device__ Map<VectorXi> fixed_nodes() { return Map<VectorXi>(d_fixed_nodes, n_constraint / 3); }

    // ================================

    __device__ Real L(int elem_idx) const { return d_L[elem_idx]; }

    __device__ Real W(int elem_idx) const { return d_W[elem_idx]; }

    __device__ Real H(int elem_idx) const { return d_H[elem_idx]; }
    ...
    void Initialize() {
        ...
        da_x_cur_jac.resize(n_coef);
        da_x_cur_jac.BindDevicePointer(&d_x_cur_jac);
        da_y_cur_jac.resize(n_coef);
        da_y_cur_jac.BindDevicePointer(&d_y_cur_jac);
        da_z_cur_jac.resize(n_coef);
        da_z_cur_jac.BindDevicePointer(&d_z_cur_jac);
    ...
    void Setup(const VectorXR& length,
        ...
        std::copy(h_x_cur.data(), h_x_cur.data() + n_coef, da_x_cur_jac.host());
        da_x_cur_jac.ToDevice();
        std::copy(h_y_cur.data(), h_y_cur.data() + n_coef, da_y_cur_jac.host());
        da_y_cur_jac.ToDevice();
        std::copy(h_z_cur.data(), h_z_cur.data() + n_coef, da_z_cur_jac.host());
        da_z_cur_jac.ToDevice();
    ...
    void RetrieveInternalForceToCPU(VectorReal3& internal_force) {
        ...
        if (!UnflattenVectorReal3(flat, internal_force)) {
            MOPHI_ERROR(
                "GPU_ANCF3443_Data::RetrieveInternalForceToCPU: internal force size (%d) is not divisible by 3.",
    ...
    int GetConstraintMode() const { return constraint_mode; }

    // ── ElementBase dispatch helpers ─────────────────────────────────────────

    ElementBase* GetDevicePtr() override { return d_data; }

    void PrepareSolverData() override { CalcDsDuPre(); }
  ...
  private:
    ...
    mophi::DualArray<Real> da_x_cur_jac, da_y_cur_jac, da_z_cur_jac;
    ...
    Real *d_x_cur_jac, *d_y_cur_jac, *d_z_cur_jac;

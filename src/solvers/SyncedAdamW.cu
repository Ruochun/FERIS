...
__global__ void one_step_adamw_kernel_impl(ElementType* d_data, SyncedAdamWSolver* d_adamw_solver) {
    ...
    if (tid < d_adamw_solver->get_n_coef()) {
        d_adamw_solver->x_prev()(tid) = d_data->x_cur()(tid);
        d_adamw_solver->y_prev()(tid) = d_data->y_cur()(tid);
        d_adamw_solver->z_prev()(tid) = d_data->z_cur()(tid);
    }
    ...
    for (int outer_iter = 0; outer_iter < d_adamw_solver->solver_max_outer(); outer_iter++) {
        if (*d_adamw_solver->outer_flag() == 0) {
            ...
            // Update positions
            if (tid < d_adamw_solver->get_n_coef()) {
                d_data->x_cur()(tid) = d_adamw_solver->x_prev()(tid) +
                                       d_adamw_solver->v_guess()(tid * 3 + 0) * d_adamw_solver->solver_time_step();
                d_data->y_cur()(tid) = d_adamw_solver->y_prev()(tid) +
                                       d_adamw_solver->v_guess()(tid * 3 + 1) * d_adamw_solver->solver_time_step();
                d_data->z_cur()(tid) = d_adamw_solver->z_prev()(tid) +
    ...
    if (tid < d_adamw_solver->get_n_coef()) {
        d_data->x_cur()(tid) =
            d_adamw_solver->x_prev()(tid) + d_adamw_solver->v_guess()(tid * 3 + 0) * d_adamw_solver->solver_time_step();
        d_data->y_cur()(tid) =
            d_adamw_solver->y_prev()(tid) + d_adamw_solver->v_guess()(tid * 3 + 1) * d_adamw_solver->solver_time_step();
        d_data->z_cur()(tid) =
            d_adamw_solver->z_prev()(tid) + d_adamw_solver->v_guess()(tid * 3 + 2) * d_adamw_solver->solver_time_step();
    }
